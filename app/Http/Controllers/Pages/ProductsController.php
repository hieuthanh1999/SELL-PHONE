...
class ProductsController extends Controller
{
  ...
  public function getProduct(Request $request, $id) {
    ...
    $suggest_products2 = Product::select('id','name', 'image', 'rate')
    ->whereHas('product_detail', function (Builder $query) {
        $query->where('quantity', '>', 0);
    })
    ->where([['producer_id', $product->producer_id], ['id', '<>', $id]])
    ...
    return view('pages.product')->with(['data' => ['advertises' => $advertises, 'product' => $product, 'product_details' => $product_details, 'suggest_products' => $suggest_products, 'product_votes' => $product_votes, 'suggest_products2'=>$suggest_products2]]);
  }
  public function compare($id, $compareId){
    $product = Product::select('id', 'producer_id', 'name', 'sku_code', 'monitor', 'front_camera', 'rear_camera', 'CPU', 'GPU', 'RAM', 'ROM', 'OS', 'pin', 'rate', 'information_details', 'product_introduction')
    ->whereHas('product_details', function (Builder $query) {
      $query->where('import_quantity', '>', 0);
    })
    ->where('id', $id)->with(['promotions' => function ($query) {
      ...
      $productCompare = Product::select('id', 'producer_id', 'name', 'sku_code', 'monitor', 'front_camera', 'rear_camera', 'CPU', 'GPU', 'RAM', 'ROM', 'OS', 'pin', 'rate', 'information_details', 'product_introduction')
      ->whereHas('product_details', function (Builder $query) {
        $query->where('import_quantity', '>', 0);
      })
      ->where('id', $compareId)->with(['promotions' => function ($query) {
        $query->select('id', 'product_id', 'content')
              ->where([['start_date', '<=', date('Y-m-d')],
                ['end_date', '>=', date('Y-m-d')]])
              ->latest();
    ...
    return view('pages.compare')->with(['data' => ['productCompare' => $productCompare, 'product' => $product]]);
  }
