          ...
          <table id="user-table" class="table table-hover" style="width:100%; min-width: 920px;">
            <thead>
              <tr>
                ...
                <th data-width="60px" data-type="date-euro">Role</th>
                @endif
            ...
            <tbody>
              @foreach($users as $user)
                <tr>
                  ...
                  <td> 
                  @if($user->active)
                  <form id="formStatus" action="{{ route('admin.users.updateLevel', ['id' => $user->id]) }}" method="POST" accept-charset="utf-8" enctype="multipart/form-data">
                    @csrf
                    <select class="form-control" name="admin" onchange="this.form.submit()">
                      <option value="0" @if( $user->admin == 0 ) selected  @endif>Khách Hàng</option>
                      <option value="2" @if( $user->admin == 2 ) selected @endif>Nhân Viên</option>
                    </select>
                  </form>
